...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <%
        ...
        ArrayList<QuestionBank> questionBanks = (ArrayList<QuestionBank>) request.getAttribute("questionBanks");
						...
						<div class="span12">
							<div class="portlet box purple">
								...
								<div class="portlet-body">
                                    ...
                                    <hr class="clearfix" />
                                    <%
                                        for(int i = 0; i <= questionBanks.size() / 4; ++i) {
                                    %>

                                            <div class="row-fluid">
                                                <%
                                                    int max;
                                                    if(i == questionBanks.size() / 4) {
                                                        max = questionBanks.size() % 4;
                                                    } else {
                                                        max = 4;
                                                    }
                                                    for(int j = 0; j < max; ++j) {
                                                        QuestionBank questionBank = questionBanks.get(i*4+j);
                                                %>
                                                        <div class="span3">
                                                            <div class="item">
                                                                <div class="zoom">
                                                                    <div class="classbackground" onclick="window.location.href='showQuestionBankInformation.do?questionBankID=<%=questionBank.getQuestionBankID()%>&type=show'" >
                                                                        <div class="textshow">
                                                                            <h3><%=questionBank.getQuestionBankName()%></h3>
                                                                        </div>
                                                                    </div>
                                                                </div>

                                                                <div class="details">
                                                                    <a href="showQuestionBankInformation.do?questionBankID=<%=questionBank.getQuestionBankID()%>&type=edit" class="icon"><i class="icon-pencil"></i></a>
                                                                    <a href="deleteQuestionBank.do?questionBankID=<%=questionBank.getQuestionBankID()%>" class="icon"><i class="icon-remove"></i></a>
                                                                </div>
                                                            </div>
                                                        </div>
                                                <%
